...
/** Forwards the verified bearer token so RLS runs as the signed-in KainFit user. */
...
/** KainFit tracks days in Manila time (UTC+8). Returns YYYY-MM-DD. */
export function manilaDate(date: Date = new Date()): string {
  return new Date(date.getTime() + 8 * 60 * 60 * 1000).toISOString().slice(0, 10);
}

/** UTC ISO bounds [start, end) for one Manila calendar day. */
export function manilaDayRange(day: string): { startIso: string; endIso: string } {
