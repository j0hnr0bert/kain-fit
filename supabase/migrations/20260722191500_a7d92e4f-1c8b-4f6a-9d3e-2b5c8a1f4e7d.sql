-- Widen is_allowed_event_name() to accept the Food Scale Guide funnel events.
-- These were already tracked client-side (analytics.ts EventName,
-- beta.functions.ts ALLOWED_EVENTS) but never added here, so every call to
-- log_product_event for them has been silently rejected by this function
-- since the Scale Guide feature shipped.
